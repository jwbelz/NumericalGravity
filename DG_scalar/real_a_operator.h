namespace Scalar_Evolution
{
  ...
  template <int dim, int degree, int n_points_1d>
  A_Operator<dim, degree, n_points_1d>::A_Operator()
  {}


  template <int dim, int degree, int n_points_1d>
  void A_Operator<dim, degree, n_points_1d>::reinit(
    const Mapping<dim> &   mapping,
    const std::vector<const DoFHandler<dim> *> &dof_handlers,
    const std::vector<const AffineConstraints<double> *> &constraints)
  {/*
    This initializes the operator with knowledge of the mesh and
    constraints. It also specifies what kind of information will be needed on the
    cell integrations.
    */
  ...
  template <int dim, int degree, int n_points_1d>
  void A_Operator<dim, degree, n_points_1d>::initialize_vector(
  ...
  template <int dim, int degree, int n_points_1d>
  void A_Operator<dim, degree, n_points_1d>::local_apply_cell(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> & a_vec,
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  {
    //This performs the integrations over cells of the rhs of a evolution equation
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        a.reinit(cell);
        a.gather_evaluate(a_vec, EvaluationFlags::values);
        alpha.reinit(cell);
        ...
        for (unsigned int q = 0; q < a.n_q_points; ++q)
          {VectorizedArray<double> r_val = a.quadrature_point(q)[0];

            a.submit_value(
              4.*numbers::PI*r_val*alpha.get_value(q)*psi.get_value(q)*pi.get_value(q)
            ,q);

          }
        a.integrate_scatter(EvaluationFlags::values,dst);
      }
  }

  template <int dim, int degree, int n_points_1d>
  void A_Operator<dim, degree, n_points_1d>::local_apply_inverse_mass_matrix(
    const MatrixFree<dim, Number> &,
    LinearAlgebra::distributed::Vector<Number> &      dst,
    const LinearAlgebra::distributed::Vector<Number> &src,
    const std::pair<unsigned int, unsigned int> &     cell_range) const
  ...
  template <int dim, int degree, int n_points_1d>
  void A_Operator<dim, degree, n_points_1d>::apply(
    ...
    LinearAlgebra::distributed::Vector<Number> &      dst) const
  {
    // Applies the operator to an input vector (src) and outputs result to dst
    {
      ...
      data.loop(&A_Operator::local_apply_cell,
                this,
                dst,
                src,
                true);
...

    {
      ...
      data.cell_loop(&A_Operator::local_apply_inverse_mass_matrix,
                     this,
                     dst,
                     dst);
    }
  ...
  template <int dim, int degree, int n_points_1d>
  void A_Operator<dim, degree, n_points_1d>::perform_stage(
    ...
    LinearAlgebra::distributed::Vector<Number> &      next_ri) const
  {
    ...
    //Then updates the solution vector
    {
      ...
      data.cell_loop(&A_Operator::local_apply_cell,
                this,
                vec_ki,
                current_ri,
                true);
    ...
    {
      data.cell_loop(
        &A_Operator::local_apply_inverse_mass_matrix,
        this,
        next_ri,
        vec_ki,
