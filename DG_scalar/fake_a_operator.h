// The operator for integrationg the a_solution outwards
namespace Scalar_Evolution
{
  ...
  template <int dim, int degree, int n_points_1d>
  fakeAOperator<dim, degree, n_points_1d>::fakeAOperator()
  {}

  template <int dim, int degree, int n_points_1d>
  void fakeAOperator<dim, degree, n_points_1d>::reinit(
    const Mapping<dim> &   mapping,
    const DoFHandler<dim> &dof_handler_DG,
    const AffineConstraints<double>  &constraint_DG)
  {/*
    Just initializes the operator with knowledge of the degrees of freedom, constraints,
    and the mesh its build on.
    In addition it specifies what kind of information will be needed for the integration*/
  ...
  template <int dim, int degree, int n_points_1d>
  void fakeAOperator<dim, degree, n_points_1d>::initialize_vector(
    LinearAlgebra::distributed::Vector<Number> &vector) const
  {
    //Just initializes the input vector so it is matched to the operator
    data.initialize_dof_vector(vector,0);
  }

  template <int dim, int degree, int n_points_1d>
  void fakeAOperator<dim, degree, n_points_1d>::fill_shape_vals(
    const FESystem<dim> &fe_DG)
  {
  ...
  template <int dim, int degree, int n_points_1d>
  void fakeAOperator<dim, degree, n_points_1d>::local_apply_cell(
    const double &r,
    const double &a_val,
    double &dr,
    const FESystem<dim> &fe_DG,
    ...
    const unsigned int &stage,
    const unsigned int &dof_interval) const
  {//This calculates the dr_a for the RK4 integration of the a solution
    ...
    // The main equation for solving for the integration of the a solutin from
    // inner boundary outwards.
    else{
      Point<dim> r_val;
      r_val[0] = r;
      double psi_val=0;
      ...
      //A much faster way for calculating the values of psi and pi at the stage points.
      for (unsigned int i=0; i<fe_DG.n_dofs_per_cell(); ++i)
      {
        psi_val += DG_shape_vals[dof_interval*(4*(fe_degree + 1)) + stage*(fe_degree + 1) + i]*psi_solution[fe_DG.n_dofs_per_cell()*cell_num + i];
        pi_val += DG_shape_vals[dof_interval*(4*(fe_degree + 1)) + stage*(fe_degree + 1) + i]*pi_solution[fe_DG.n_dofs_per_cell()*cell_num + i];
      }
      //Here we actually calculate dr_a for the RK4 integration
      dr = a_val*((1-a_val*a_val)/(2.*r) + 2*numbers::PI*r*(psi_val*psi_val + pi_val*pi_val));
    }
  }

  template <int dim, int degree, int n_points_1d>
  void fakeAOperator<dim, degree, n_points_1d>::perform_stage(
    const double                                      current_position,
    const double                                      /*factor_solution*/,
    const double                                      h_,
    const double &current_ri,
    double &      solution,
    double &      /*next_ri*/,
    const FESystem<dim>                               &fe_DG,
    const unsigned int                                &dof_num,
    DoFHandler <dim>                                  &dof_handler) const
  {
    //The driver of runge kutta solving the a equation
    double k;
    double ki = 0.;
    ...
    unsigned int dof_interval = dof_num % fe_DG.n_dofs_per_cell() - 1;
      ...
      local_apply_cell(current_position,current_ri,k,fe_DG,cell_num,dof_handler,0,dof_interval);
      ki += k;
      local_apply_cell(current_position+0.5*h_, current_ri+k*0.5*h_,k,fe_DG,cell_num,dof_handler,1,dof_interval);
      ki += 2.*k;
      local_apply_cell(current_position+0.5*h_, current_ri+k*0.5*h_,k,fe_DG,cell_num,dof_handler,2,dof_interval);
      ki += 2.*k;
      local_apply_cell(current_position+h_, current_ri + h_*k, k,fe_DG,cell_num,dof_handler,3,dof_interval);
      ki += k;
